...
impl AlgorandConstant {
    ...
    pub fn value(&self) -> u64 {
        match self {
            ...
            AlgorandConstant::MaxTxnGroupSize => MAX_TXN_GROUP_SIZE as u64,
        }
    }
}
...
mod tests {
    ...
    fn test_group_transactions_ffi() {
        ...
        for (original_txn, grouped_txn) in txs.iter().zip(grouped_txs.iter()) {
            assert_eq!(original_txn.group, None);
            assert_eq!(grouped_txn.group.as_ref().unwrap(), &expected_group);
        }
    }
}
