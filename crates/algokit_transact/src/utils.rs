...
pub fn compute_group_id(txns: &[Transaction]) -> Result<Byte32, AlgoKitTransactError> {
    let raw_ids: Result<Vec<Byte32>, AlgoKitTransactError> = txns
        .iter()
        .map(|txn| {
            let already_grouped = match txn {
                Transaction::Payment(fields) => fields.header.group.is_some(),
                Transaction::AssetTransfer(fields) => fields.header.group.is_some(),
            };
            if already_grouped {
                return Err(AlgoKitTransactError::InputError(
                    "Transactions must not be already grouped".to_string(),
                ));
            }
            txn.raw_id()
        })
