...
/// struct for typed errors of method [`state_proof`]
...
pub async fn state_proof(http_client: &dyn HttpClient, round: u64) -> Result<StateProof, Error> {
    let p_round = round;

    let path = format!("/v2/stateproofs/{round}", round = p_round);
