    ...
    public static stringify(data: any, mediaType: string, typeHint?: string): any {
        ...
        if (isMsgpackMimeType(mediaType)) {
            try {
                ...
                const modelEnum = typeHint && (algokit_msgpack.listSupportedModelsFfi() as unknown as string[]).indexOf(typeHint) !== -1
                    ? typeHint as algokit_msgpack.ModelType
                    : undefined;

                return modelEnum ? algokit_msgpack.encodeJsonToMsgpackFfi(modelEnum, jsonStr) : jsonStr;
            } catch (err) {
                console.error(err);
    ...
    public static parse(rawData: any, mediaType: string | undefined, typeHint?: string) {
        ...
        if (isMsgpackMimeType(mediaType)) {
            try {
                ...
                const jsonStr = algokit_msgpack.decodeMsgpackToJsonFfi(modelEnum, bytes);
