...
class AccountApplicationInformation200Response(BaseModel):
    """
    ...
    app_local_state: Optional[ApplicationLocalState] = Field(default=None, alias="app-local-state")
    created_app: Optional[ApplicationParams] = Field(default=None, alias="created-app")
    __properties: ClassVar[List[str]] = ["round", "app-local-state", "created-app"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of app_local_state
        if self.app_local_state:
            _dict['app-local-state'] = self.app_local_state.to_dict()
        # override the default output from pydantic by calling `to_dict()` of created_app
        if self.created_app:
            _dict['created-app'] = self.created_app.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "round": obj.get("round"),
            "app-local-state": ApplicationLocalState.from_dict(obj["app-local-state"]) if obj.get("app-local-state") is not None else None,
            "created-app": ApplicationParams.from_dict(obj["created-app"]) if obj.get("created-app") is not None else None
        })
        return _obj
