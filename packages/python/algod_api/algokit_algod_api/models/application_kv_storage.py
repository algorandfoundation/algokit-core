...
class ApplicationKVStorage(BaseModel):
    """
    An application's global/local/box state.
    """ # noqa: E501
    kvs: List[AvmKeyValue] = Field(description="Key-Value pairs representing application states.")
    account: Optional[StrictStr] = Field(default=None, description="The address of the account associated with the local state.")
    __properties: ClassVar[List[str]] = ["kvs", "account"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in kvs (list)
        _items = []
        if self.kvs:
            for _item_kvs in self.kvs:
                if _item_kvs:
                    _items.append(_item_kvs.to_dict())
            _dict['kvs'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "kvs": [AvmKeyValue.from_dict(_item) for _item in obj["kvs"]] if obj.get("kvs") is not None else None,
