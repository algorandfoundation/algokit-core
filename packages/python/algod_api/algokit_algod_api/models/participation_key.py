...
class ParticipationKey(BaseModel):
    """
    Represents a participation key used by the node.
    """ # noqa: E501
    id: StrictStr = Field(description="The key's ParticipationID.")
    address: StrictStr = Field(description="Address the key was generated for.")
    effective_first_valid: Optional[StrictInt] = Field(default=None, description="When registered, this is the first round it may be used.", alias="effective-first-valid")
    effective_last_valid: Optional[StrictInt] = Field(default=None, description="When registered, this is the last round it may be used.", alias="effective-last-valid")
    last_vote: Optional[StrictInt] = Field(default=None, description="Round when this key was last used to vote.", alias="last-vote")
    last_block_proposal: Optional[StrictInt] = Field(default=None, description="Round when this key was last used to propose a block.", alias="last-block-proposal")
    last_state_proof: Optional[StrictInt] = Field(default=None, description="Round when this key was last used to generate a state proof.", alias="last-state-proof")
    key: AccountParticipation
    __properties: ClassVar[List[str]] = ["id", "address", "effective-first-valid", "effective-last-valid", "last-vote", "last-block-proposal", "last-state-proof", "key"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "effective-first-valid": obj.get("effective-first-valid"),
            "effective-last-valid": obj.get("effective-last-valid"),
            "last-vote": obj.get("last-vote"),
            "last-block-proposal": obj.get("last-block-proposal"),
            "last-state-proof": obj.get("last-state-proof"),
            "key": AccountParticipation.from_dict(obj["key"]) if obj.get("key") is not None else None
        })
        return _obj
